...
class RSyslogAnalyzer:
    ...
    def load_logs(self):
        ...
        # Try multiple patterns to handle different syslog formats
        patterns = [
            r'^(?P<month>\w{3})\s+(?P<day>\d+)\s+(?P<time>\d{2}:\d{2}:\d{2})\s+(?P<host>[\w\-.]+)\s+(?P<service>[\w\-.\/]+)(?:\[\d+\])?:\s(?P<message>.+)$',
            r'^(?P<month>\w{3})\s+(?P<day>\d+)\s+(?P<time>\d{2}:\d{2}:\d{2})\s+(?P<host>[\w\-.]+)\s+(?P<service>[\w\-.\/]+):\s(?P<message>.+)$',
            r'^(?P<month>\w{3})\s+(?P<day>\d+)\s+(?P<time>\d{2}:\d{2}:\d{2})\s+(?P<host>[\w\-.]+)\s+(?P<service>[\w\-.\/]+):\s\[(?P<level>[A-Z]+)\]\s(?P<message>.+)$'
        ]
                ...
                for line in f:
                    for pattern in patterns:
                        ...
                        if match:
                            ...
                            try:
                                # Get current year from timestamp if possible, otherwise use current year
                                ts = datetime.strptime(ts_str, "%b %d %H:%M:%S")
                                if 'timestamp' in match.groupdict():
                                    ts = ts.replace(year=ts.year)
                                else:
